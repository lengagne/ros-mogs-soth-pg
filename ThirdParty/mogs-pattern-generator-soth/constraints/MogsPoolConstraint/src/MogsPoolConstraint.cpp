...
#include "MogsPoolConstraint.h"

MogsPoolConstraint::MogsPoolConstraint(	QDomElement pg_root,
								RigidBodyDynamics::MogsKinematics<F<double> >* kin,
								std::vector< RigidBodyDynamics::MogsKinematics< double > * >* all_the_robots )
{
	QDomElement ElBody1 = pg_root.firstChildElement("fixed_body");
	if (ElBody1.isNull())
	{
	...
	QDomElement ElBody2 = pg_root.firstChildElement("movable_body");
	if (ElBody2.isNull())
	{
		std::cerr<<"Error : you must defined a body in the constraint position_effector"<<std::endl;
		exit(0);
...
MogsPoolConstraint::~MogsPoolConstraint()
{

}

void MogsPoolConstraint::compute(	Eigen::Matrix <F<double>,Eigen::Dynamic, 1 > &Q,
						RigidBodyDynamics::MogsKinematics<F<double> > * kin,
...
extern "C" MogsPoolConstraint* create(   QDomElement pg_root,
                                                RigidBodyDynamics::MogsKinematics<F<double> >* kin,
                                                std::vector< RigidBodyDynamics::MogsKinematics< double > * >* all_the_robots)
{
    return new MogsPoolConstraint(pg_root, kin, all_the_robots);
}

extern "C" void destroy(MogsPoolConstraint* p)
{
    delete p;
}
